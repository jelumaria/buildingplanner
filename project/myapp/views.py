...
def equipments_details(request):
    e =  Equipment.objects.all()
    context={'details':e}
    return render(request, './myapp/equipments-details.html',context)
...
def architect_logout(request):
    try:
        ...
        return architect_login(request)
    else:
        return architect_login(request)
...
from .emailtest import send_mail
def user_forgot(request):
    if request.method == 'POST':
        uname = request.POST.get('uname')

        ul = user_login.objects.filter(uname=uname, u_type='user')
        print(len(ul))
        if len(ul) == 1:
            send_mail("Your Password", "your password is"+ ul[0].passwd, uname)
            #send_mail('Login','welcome'+uname,uname)
            context = {'msg':'Password send to email id'}
            return render(request, 'myapp/user_login.html',context)
        else:
            context = {'msg': 'Invalid Credentials'}
            return render(request, 'myapp/user_login.html',context)
    else:
        return render(request, 'myapp/user_forgot.html')
...
def user_details_add(request):
    if request.method == 'POST':
        ...
        ud = user_details(user_id=user_id,fname=fname, lname=lname, gender=gender,addr=addr, pin=pin, contact=contact, email=email )
        ud.save()
...
def user_plans_search(request):
    ...
    if request.method == 'POST':
        ...
        fpm_l.extend(pm_l)
        pm_l2 = architect_plans.objects.filter(descrp__contains=query)
        fpm_l.extend(pm_l2)
        scm_l = plan_settings.objects.all()
        ar_l = architect_details.objects.all()
        context = {'architect_list':ar_l,'plan_list': scm_l, 'ar_plan_list': fpm_l, 'msg': ''}
...
def user_architect_proposal_add(request):
    if request.method == 'POST':
        ...
        pm = user_proposal(user_id=int(user_id),filepath=filepath,
                           requirments=requirments,remark=remark,dt=dt,tm=tm,status='ok' )
        pm.save()

        context = {'msg':''}
        return redirect("http://127.0.0.1:8000/myapp/user_home")
        ...
        try:
            ...
            if os.path.exists(document_path):
                with open(document_path, 'rb') as file:
                    ...
                    return response
        except (ValueError, user_proposal.DoesNotExist):
            pass
...
def admin_user_details(request):
    ...
    return render(request, './myapp/admin_user_details.html', context)

def admin_architect_details(request):
    jm_l = architect_details.objects.all()
    context = {'architect_list': jm_l}
    return render(request, './myapp/admin_architect_details.html', context)
